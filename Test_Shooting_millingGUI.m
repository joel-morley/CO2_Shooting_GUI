...
% This section creates GUI checkboxes well before the rest of the GUI is built so
% that the checkbox values are set before the camera starts
% Create RCheckBox
...
%                     hold off
        ...
        % Button pushed function: 2D fit
        function D2_fitButtonPushed(src,event)
            hdata = guidata(fig);
            D2_fit_GUI(hdata.surf);
            guidata(fig,hdata)
        end
...
function plot_milling()% plot the milling pattern over the camera view
...
%             AutoFocusButton.Text = {'Auto-Focus'; ''};
            ...
            % Create 2D Fit Button
            D2_fitButton = uibutton(fig,'ButtonPushedFcn', @D2_fitButtonPushed);
            D2_fitButton.FontSize = 20;
            D2_fitButton.Position = [992 25 70, 56];
            D2_fitButton.Text = '2D Fit';
